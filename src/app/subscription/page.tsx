...
// Removed KortixLogo import
import { createClient } from '@/lib/supabase/client';
import { clearUserLocalStorage } from '@/lib/utils/clear-local-storage';
// import { useMaintenanceNoticeQuery } from '@/hooks/react-query/edge-flags'; // Removed for static landing page
...
export default function SubscriptionRequiredPage() {
  ...
  // const { data: maintenanceNotice, isLoading: maintenanceLoading } = useMaintenanceNoticeQuery(); // Removed for static landing page
  const maintenanceNotice = { enabled: false }; // Always disabled for static landing page
  const maintenanceLoading = false; // Never loading for static landing page
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        <div className="text-center space-y-4">
          ...
          <p className="text-md text-muted-foreground max-w-2xl mx-auto">
            {isTrialExpired
              ? 'Your 7-day free trial has ended. Choose a plan to continue using Dimatic AI.'
              : 'A subscription is required to use Dimatic. Choose the plan that works best for you.'}
          </p>
        </div>
        <PricingSection
        ...
        <div className="text-center text-sm text-muted-foreground -mt-10">
          <p>
            ...
            <a href="mailto:support@dimatic.ai" className="underline hover:text-primary">
              support@dimatic.ai
            </a>
          </p>
        </div>
