...
interface AgentSelectionState {
  ...
  isDimaticAgent: (agents: Agent[]) => boolean;
}
  ...
  persist(
    (set, get) => ({
      ...
      initializeFromAgents: (agents: Agent[], threadAgentId?: string, onAgentSelect?: (agentId: string | undefined) => void) => {
        ...
        if (!selectedId) {
          ...
          if (current && agents.some(a => a.agent_id === current)) {
            ...
            const defaultDimaticAgent = agents.find(agent => agent.metadata?.is_dimatic_default);
            selectedId = defaultDimaticAgent ? defaultDimaticAgent.agent_id : agents[0].agent_id;
          }
        }
      ...
      autoSelectAgent: (agents: Agent[], onAgentSelect?: (agentId: string | undefined) => void, currentSelectedAgentId?: string) => {
        ...
        const defaultDimaticAgent = agents.find(agent => agent.metadata?.is_dimatic_default);
        const agentToSelect = defaultDimaticAgent || agents[0];
      ...
      isDimaticAgent: (agents: Agent[]) => {
        const { selectedAgentId } = get();
        const currentAgent = selectedAgentId
          ? agents.find(agent => agent.agent_id === selectedAgentId)
          : null;
        return currentAgent?.metadata?.is_dimatic_default || selectedAgentId === undefined;
      },
    }),
    {
...
export const useAgentSelection = () => {
  ...
  return {
    ...
    isDimaticAgent: store.isDimaticAgent,
  };
};