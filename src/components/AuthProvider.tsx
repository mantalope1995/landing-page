...
import { checkAndInstallDimaticAgent } from '@/lib/utils/install-dimatic-agent';
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange(
      async (event, newSession) => {
        ...
        switch (event) {
          case 'SIGNED_IN':
            if (newSession?.user) {
              await checkAndInstallDimaticAgent(newSession.user.id, newSession.user.created_at);
            }
            break;
          case 'SIGNED_OUT':
